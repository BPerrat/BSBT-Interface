...
def generate_cluster_images(data):
    ...
    # Compute the extent of the island containing the cluster
    #c = [cluster if len(cluster) == 1 else cluster.iloc[0]]
    #extent = islands[cluster.centroid.within(islands)].envelope
    ...
    ctx.add_basemap(ax, crs=df.crs.to_string(), attribution='')  #, zoom=12)
    plt.axis('off')
